        ...
        const checkForSyncCallbacks = (node) => {
            ...
            if (node.callee.type === "Identifier") {
                if (matchImport(["createSignal", "createStore"], node.callee.name) &&
                    ...
                    const setter = getNthDestructuredVar(node.parent.id, 1, context.sourceCode.getScope(node.parent));
                    if (setter) {
        ...
        const checkForReactiveAssignment = (id, init) => {
            ...
            if (init.type === "CallExpression" && init.callee.type === "Identifier") {
                ...
                if (matchImport(["createSignal", "useTransition"], callee.name)) {
                    ...
                    const resourceReturn = id && getNthDestructuredVar(id, 0, context.sourceCode.getScope(init));
                    if (resourceReturn) {
                ...
                if (node.callee.type === "Identifier") {
                    ...
                    if (matchImport([
                        ...
                        if (arg1) {
                            ...
                            const owner = arg0.type === "Identifier" && findVariable(context.sourceCode.getScope(node), arg0);
                            if (owner) {
